...
// Server-side DataTables endpoint: GET /admin/queues-dt
app.get('/admin/queues-dt', adminAuth, async (req, res) => {
  try {
    const draw = Number(req.query.draw || 0);
    const start = Number(req.query.start || 0);
    const length = Math.min( Number(req.query.length || 25), 500 );
    const searchValue = (req.query['search[value]'] || '').toString().toLowerCase();

    // Mapping kolom index -> nama field
    const orderCol = Number(req.query['order[0][column]'] || 2); // default ke pendingCount
    const orderDir = (req.query['order[0][dir]'] || 'desc').toLowerCase() === 'asc' ? 'asc' : 'desc';
    const columnMap = {
      0: 'userId',
      1: 'queueName',
      2: 'pendingCount',
    ...
    const all = db.listQueueArray();
    const recordsTotal = all.length;

    // Filter by search (userId / queueName / consumerStatus)
    const filteredMeta = all.filter((q) => {
      if (!searchValue) return true;
      const s = searchValue;
      return (
        (q.userId || '').toLowerCase().includes(s) ||
        (q.queueName || '').toLowerCase().includes(s) ||
        ((q.consumerStatus || 'stopped').toLowerCase().includes(s))
      );
    });
    const recordsFiltered = filteredMeta.length;

    // Sorting memakai metadata + placeholder pendingCount (akan dihitung untuk page saja)
    const sortField = columnMap[orderCol] || 'pendingCount';
    ...
    // Pagination
    const pageMeta = filteredMeta.slice(start, start + length);

    // Hitung stats (pendingCount/consumers) hanya untuk baris di halaman ini
    const data = [];
    for (const q of pageMeta) {
      const stats = await qm.getQueueStats(q.queueName);
      data.push({
        userId: q.userId,
        queueName: q.queueName,
...
// Ringkasan ringan: tanpa menghitung totalPending (agar tidak berat)
app.get('/admin/summary', adminAuth, async (req, res) => {
  try {
    const arr = db.listQueueArray();
    const totalQueues = arr.length;
    const activeConsumers = arr.filter((x) => (x.consumerStatus || 'stopped') === 'started').length;
    return res.json({ success: true, totalQueues, activeConsumers, totalPending: null });
  } catch (e) {
    return res.status(500).json({ error: e.message || 'failed' });
  }
});
